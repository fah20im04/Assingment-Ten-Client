  ...
  return (
    <div className="relative w-full bg-gray-500 dark:bg-darkBg dark:text-white">
      ...
      {menuOpen && (
        <div className="md:hidden absolute text-black dark:text-gray-300 top-20 left-2/3 transform -translate-x-1/2 w-[30%] max-w-xs rounded-2xl shadow-xl border border-gray-600 z-30 bg-white dark:bg-darkCard">
          <ul className="flex flex-col gap-1 p-3 font-semibold">
            ...
            {user && (
              <li>
                <NavLink to="/addVehicle" onClick={() => setMenuOpen(false)}>
                  Add Vehicle
                </NavLink>
              </li>
            )}
            <li>
              <NavLink to="/allVehicles" onClick={() => setMenuOpen(false)}>
                All Vehicles
              </NavLink>
            </li>
            {user && (
              <>
                <li>
                  <NavLink to="/myVehicles" onClick={() => setMenuOpen(false)}>
                    My Vehicles
                  </NavLink>
                </li>
                <li>
                  <NavLink to="/myBookings" onClick={() => setMenuOpen(false)}>
                    My Bookings
                  </NavLink>
                </li>
              </>
      ...
      <div className="flex justify-between items-center px-6 md:px-12 py-4 z-20 relative">
        ...
        {/* Desktop menu */}
        <div className="hidden md:flex bg-opacity-80 p-4 rounded-2xl w-fit shadow-md flex border border-gray-300 gap-6 font-semibold text-black dark:text-white text-sm h-10">
